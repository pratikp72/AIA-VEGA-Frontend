...
function buildPeopleProfileHref(person) {
  if (!person.name) return '/people';
  const params = new URLSearchParams();
  params.set('search', person.name);
  return `/people?${params.toString()}`;
}

function hasWelcomeNote(person) {
  return Boolean(String(person.welcomeNote || '').trim());
}
  ...
  return (
    <section className="flex flex-col min-h-0">
      ...
      <div className="flex flex-col items-start gap-6 w-full overflow-y-auto scrollbar-hide" style={{ maxHeight: '632px' }}>
        {joinees.map((person) => {
          const showReadMore = hasWelcomeNote(person);
          const peopleHref = buildPeopleProfileHref(person);
          const welcomeHref = `/people/new-joinee/${person.id}`;

          return (
            <div key={person.id} className="relative block w-full min-w-0">
              <Link href={peopleHref} className="block w-full min-w-0">
              ...
              {showReadMore ? (
                <Link
                  href={welcomeHref}
                  className="absolute right-4 top-4 z-10 inline-flex items-center gap-1 text-primary-purple hover:text-primary"
                  aria-label={`Read welcome note for ${person.name}`}
                >
                  <BookOpen className="h-4 w-4 shrink-0" />
                  <span className="text-small text-muted-foreground truncate">Read Note</span>
                </Link>
              ) : null}
            </div>
          );
