...
function normalizeRoute(route) {
  ...
  return {
    id: attrs.stop_id ?? route?.stop_id ?? route?.id ?? null,
    routeId: attrs.stop_id ?? route?.stop_id ?? null,
    name: attrs.stop_name ?? attrs.name ?? '',
    locationLink: attrs.bus_stop_location_link ?? route?.bus_stop_location_link ?? '',
...
function normalizeUnitWithRoutes(u) {
  ...
  const image = attrs.route_img ?? u?.route_img ?? attrs.unit_img ?? u?.unit_img;
  const rawRoutes = attrs.bus_stops ?? u?.bus_stops ?? attrs.routes ?? u?.routes ?? [];
  const routesArr = Array.isArray(rawRoutes) ? rawRoutes : rawRoutes?.data ?? [];
  return {
    id: u?.id ?? u?.documentId ?? attrs.route_id ?? attrs.unit_id ?? null,
    unitId: attrs.route_id ?? attrs.unit_id ?? null,
    unitName: attrs.route_name ?? attrs.unit_name ?? attrs.name ?? '',
    address: attrs.address ?? '',
    contact: attrs.contact ?? '',
    siteManager: attrs.site_manager ?? '',
    hrManager: attrs.hr_manager ?? '',
    mapLink: attrs.route_map_link ?? attrs.map_link ?? '',
...
export const fetchLocationRoutes = async (locationId) => {
  ...
  const res = await apiService.get(API_ENDPOINTS.LOCATION.UNIT_LOCATION_BY_ID(locationId), {
    params: {
      ...
      'populate[routes][populate][bus_stops][populate][bus_sifts]': true,
  ...
  const unitsRaw = data?.routes ?? data?.attributes?.routes ?? data?.units ?? data?.attributes?.units ?? [];
