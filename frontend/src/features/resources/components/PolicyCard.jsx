  ...
  return (
    ...
    >
      <div className="flex items-center gap-2">
        <h4 className="text-xl font-bold leading-tight max-w-[80%] truncate ">{item.title}</h4>
        {Array.isArray(item.tags) && item.tags.length > 0 ? (
          <span className="shrink-0 inline-flex items-center bg-primary-purple text-white text-xs font-medium px-3 py-1 rounded-[8px]">
            {item.tags.join(', ')}
          </span>
        ) : item.tags && typeof item.tags === 'string' ? (
          <span className="shrink-0 inline-flex items-center bg-primary-purple text-white text-xs font-medium px-3 py-1 rounded-[8px]">{item.tags}</span>
        ) : null}
      </div>
